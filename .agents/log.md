...
## 2026-08-03 — Claude — Formal /code-review of the above found 5 real bugs, fixed 3 (commit `157fd9c`)

Filip ran `/code-review` on commit `79b05e2` right after my own self-audit. 8 finder angles (line-by-line, removed-behavior, cross-file, reuse, simplification, efficiency, altitude, CLAUDE.md conventions) + 1-vote verification found more, and worse, than my self-audit caught — including a correction to something I stated confidently in the entry directly above this one: I wrote "a bounded one's own end date is irrelevant once the vehicle is removed" about vehicle soft-delete — **that was wrong**, see bug 3 below.

Filip chose to fix only the most severe (data mismatch/silent-wrong-state risk), explicitly leaving the other two as a documented known gap rather than doing the larger UI work they'd need.

**Fixed:**

1. **Editing a CSV-imported assignment could fail outright.** The "find the assignment being edited" lookup matched on `vehicle_id + valid_from + customer_id`. CSV/XML import passes the raw cell string straight through with no truncation, so an imported row's `valid_from` can carry seconds; the edit form's `datetime-local` input is minute-precision only. Editing such a row (even just to add a `validTo`) recomputed a `valid_from` that no longer matched the stored one bit-for-bit, missed the row entirely, and fell into the "new assignment" branch.
2. **Editing could silently patch an unrelated, long-CLOSED historical assignment instead of the real current one** — same root cause (matching by fields, not identity). Same customer + same start date as some old closed row (most realistic via CSV re-import) meant that dead row got its `valid_to` patched while the vehicle's actual current assignment sat there untouched, with zero error raised anywhere.
3. **Vehicle soft-delete didn't close an assignment with an already-planned future end date.** `DELETE /api/fleet/vehicles/[id]` only closed rows with `valid_to IS NULL`. Since assignments can carry a future `valid_to` while still active (the whole point of the previous feature), deleting a vehicle mid-contract left that assignment's far-future end untouched — `matchVehicleCustomer` would keep attributing mandate events to that customer on a vehicle no longer in the fleet. This state literally couldn't exist before the previous commit.

**Fix for 1+2**: `GET /api/fleet/vehicles` now returns each assignment's `id`. `openEditVehicle` carries it through as `assignmentId`; `POST` looks the row up by that id (exact identity) when present, falling back to the old `valid_from` heuristic only when absent (add-vehicle, CSV/XML import — neither has an id to send). The heuristic itself is also tightened to never match an already-closed row (only open or currently-active), closing the CSV-reimport trigger for bug 2 as well, not just the UI-edit trigger. Reassigning a *known* (by-id) assignment to a different customer now closes that exact row directly instead of relying on the generic "any open assignment" guess — which also means reassigning a *bounded* assignment now works at all (the guess is scoped to `valid_to IS NULL`, blind to bounded rows) — and gives a specific "start date must be after the assignment being closed" error instead of a confusing generic overlap message when applicable. This incidentally also fixed the "change customer, same date" confusing-error finding from my own earlier self-audit.

**Fix for 3**: the soft-delete close-query now uses the same "active right now" definition (`valid_from <= now <= valid_to`) already used in `GET`, instead of `valid_to IS NULL` alone. A not-yet-started future assignment is deliberately left alone — out of scope, ties into the two skipped findings below.

**Also fixed while touching this code**: the pre-existing "close the open assignment" failure path (used when a genuinely new assignment supersedes an open one) didn't translate `isOverlapViolation` — always fell back to a generic 502 unlike the other two write paths. Now consistent.

**Deliberately left unfixed, per Filip's explicit scope choice** — real, confirmed findings, not chased further:
- **A future-dated, open-ended assignment is invisible in the UI.** `GET`'s "current = covers this instant" redefinition (from the previous commit) means a vehicle booked for next week with no end date shows as unassigned until its start date arrives — and since `openEditVehicle` seeds the edit form purely from that one field, there is no way to view, adjust, or cancel that pending booking before it starts. Would need either restoring some visibility for future rows in `GET` (careful: mustn't reintroduce the "shows the wrong customer as current" bug this whole feature was built to fix) or a dedicated "upcoming reservations" view.
- **Consequence of the above**: trying to book that same vehicle *immediately* for a different customer, without realizing the hidden future booking exists, hits the backdating guard and gets "Nowe przypisanie musi zaczynać się po rozpoczęciu obecnego najmu" — worded as if a rental is already running, when nothing has actually started yet.
- Two lower-severity/cleanup findings (the "active assignment" concept is hand-rolled independently in 5 places across this file + `vehicle-match.ts` + `authority-context.ts`; `isOverlapViolation`'s raw substring match on the error body is fragile to a future Supabase error-format change) — logged, not acted on.

`tsc --noEmit`, eslint, and a full `npm run build` all clean before push.

## 2026-08-06 — Claude (separate session, Windows machine) — Full status audit, OCR-auth refactor pushed to a stale branch (not on main, has a real conflict), SUPABASE_DB_URL confirmed missing

Filip asked for a full feature-by-feature status audit of the whole app (previous "what's left" summary felt too thin for the actual scope) and separately asked me to finish/push local uncommitted work another agent had left in `C:/temp/ProcesorMandatow`, checked out on branch `codex/api-diagnostics-hardening`.

**Audit**: read `workspace.tsx`, all `api/**` routes, `docs/roadmap-automatyzacje.md`, `docs/namierzanie-kierowcy.md`, `docs/mvp-test-runbook.md`, `docs/production-security-checklist.md` via parallel subagents (read-only `git show origin/main:<path>`, no working-tree changes). Notable *new* findings not previously logged here:
- **Mandate financial fields have full backend+DB support (`amountGross`, `paymentDueAt`, `financialStatus`, confirmed via the `api/health` schema-drift canary) but zero UI form fields in `workspace.tsx` to enter them** — grepped for every field name, zero hits in the component. Backend-only, unusable today.
- **`drivers.license_number`/`license_valid_until` are dead columns** — exist in the schema and in the `DriverRow` type (`api/fleet/drivers/route.ts`), never returned by any GET response, never rendered in `employees.tsx`.
- Everything else matched what's already in this log / `stan-projektu.md` (route-planner `planned_for` bug, vehicle-busy-lock gap, reorder-mid-route UI gap, Resend parked, zero tests) — no new bugs found there, just re-confirmed.

**Finished + pushed the other agent's WIP** (was uncommitted, `git status` showed modified `mandate-ocr.ts` + untracked `google-workload-identity.ts`): that work extracted the Vercel-OIDC→GCP token exchange out of `mandate-ocr.ts` into a shared `src/lib/google-workload-identity.ts`, adding `service_account_impersonation_url` (the bare `external_account` identity lacked direct Document AI permissions). Wired `documentAiConfig()`/`readWithDocumentAi()` to the new module, added `GOOGLE_WIF_SERVICE_ACCOUNT_EMAIL` to `.env.example`. Also fixed an unrelated pre-existing Turbopack build error in `api/documents/[id]/send-review-package/route.ts` — it re-exported `runtime` from another route file, which Next 16 rejects (route config must be declared per-file, not re-exported; re-exporting the handler function itself, e.g. `POST`, is fine). Two commits: `faf738c`, `f6533c2`.

**Important — these 2 commits are NOT on `main` yet.** `codex/api-diagnostics-hardening`'s prior history (everything up to `6b1fecf`) is already an ancestor of `main` (confirmed: `git merge-base --is-ancestor 6b1fecf origin/main` → yes, folded in via the `87036ff` merge), so the branch itself isn't meaningfully diverged — but my 2 new commits are, and **cherry-picking them onto `main` conflicts in `mandate-ocr.ts`**, because `main` already has a different, more advanced version of that file (the DE/FR/ES authority-detection work logged above) that this old branch never saw. This is exactly the "two agents built the same OIDC integration independently" trap this log already warns about, one level deeper — same file, second collision. **Next agent: re-apply the workload-identity extraction (same idea — pull the OIDC exchange + impersonation into `google-workload-identity.ts`) against `main`'s current `mandate-ocr.ts`, don't try to merge/cherry-pick the old branch's version.** I did not attempt this myself — ran out of turn time and didn't want to push an unverified conflict resolution to `main` (which auto-deploys) without a clean `tsc`/`eslint`/`npm run build` pass first, per this repo's own rule.

**`SUPABASE_DB_URL` — confirmed empirically missing**, not just per old docs: `vercel env ls` (scope `dashccseo`, project `procesor-mandatow`) lists no such key. However **the Vercel↔Supabase native integration already created `POSTGRES_URL` / `POSTGRES_URL_NON_POOLING` / `POSTGRES_PASSWORD` etc. (added 10 days ago)** pointing at the same database — the fastest fix is copying `POSTGRES_URL_NON_POOLING`'s value into a new `SUPABASE_DB_URL` var in the Vercel dashboard, no need to fetch a fresh password from the Supabase dashboard. Did not do this myself (didn't want to touch env vars without Filip driving it directly).

**Housekeeping, unresolved**: two untracked junk files still sit in `C:/temp/ProcesorMandatow` — `iphone-scanner-landscape.png` / `iphone-scanner-portrait.png` are accidental screenshots of a Chrome "ERR_NETWORK_IO_SUSPENDED" error page (laptop went to sleep mid-capture), not real scanner UI. Filip hasn't decided delete-vs-keep yet.

Separately (different repo, mentioned here only because it came up in the same session): Filip's new project **Flotium** (`github.com/FilipSeoCC/Flotium`, car-rental SaaS demo) was pushed to GitHub and verified running locally — unrelated to FlotaFlow except that its `.env.local` (real Supabase credentials, being wired up by another agent) uses a similar variable-name template to this project's, which caused some cross-project confusion mid-session (a screenshot of Flotium's `.env.local` was initially mistaken for this project's).

## 2026-08-11 10:37 — Codex — Local Graphify code graph for Codex and Claude
- Installed Graphify `0.9.39` through an isolated `uv` tool environment with the optional SQL parser; it is development tooling only and was not added to `package.json`, Next.js, Vercel, or the production runtime.
- Registered project-scoped Graphify skills for Codex (`.codex/skills/graphify`) and Claude (`.claude/skills/graphify`) and added the generated usage rules to `AGENTS.md` and `CLAUDE.md`. Existing `.claude/` remains gitignored as local Claude configuration.
- Added `.graphifyignore` to exclude credentials, generated output, local uploads/exports, and document/image formats that could contain personal data. `graphify-out/` and the machine-specific Codex hook configuration are intentionally gitignored.
- Windows refused to execute Graphify's generated `.exe` shim directly, so both local hook configs were changed to the verified `python.exe -m graphify ...` invocation. Do not replace it with the generated shim without testing on this machine.
- Built and tested a code-only graph without an LLM or external API: 551 nodes, 1166 links, 39 communities. A test query correctly connected the document upload route, OCR queue, `processMandateOcr`, authority detection, vehicle matching, Supabase authorization, and audit modules.
- The unrelated onboarding changes already present on `codex/user-onboarding` were not modified, staged, committed, or pushed as part of this setup.

## 2026-08-11 — Codex — Authentication hardening, MFA and onboarding branch
- Preserved Filip's account model: self-registration stays open with mandatory e-mail confirmation; the new account is `user/pending` and receives no application access until a boss or admin assigns a role and activates it.
- Removed every client-side Supabase session-token lookup from `localStorage`. Same-origin API calls now rely on `ff-access`/`ff-refresh` HttpOnly, Secure, SameSite cookies only.
- Added database-backed rate limiting for login, reset, recovery completion, password change and MFA. The limiter uses HMAC-hashed IP/e-mail/user keys and an atomic Supabase RPC. It deliberately fails open with an error log during migration/database outages because Supabase Auth's provider limit remains in place and a missing migration must not lock out all staff.
- Added 12–128 character password policy, local common-password rejection, and Have I Been Pwned k-anonymity checks. Profile password changes now require the current password and verify it server-side before applying the new password.
- Added mandatory TOTP MFA for `admin` and `boss`, including enrollment QR, challenge/verify flow, temporary HttpOnly pre-MFA cookies, AAL2 checks in the server authorization layer, and matching AAL2-aware RLS helper functions. A regular user is not forced to enroll; if that user already has a verified factor, AAL2 is enforced consistently.
...
## 2026-08-12 — Claude — Auth-check ordering fix in routes/optimize + shared graphify skill for Claude Code

- Independently arrived at the same auth-hardening/PR-#2-cleanup work Codex had already pushed (`0780626`, `46c714a`) — discovered the overlap before pushing my larger branch, discarded it, and ported only the one gap neither of us had covered: `POST /api/routes/optimize` checked the demo-mode/missing-config fallback *before* `verifyMember()`, so an unauthenticated caller could get an optimized route whenever `ROUTE_OPTIMIZATION_DEMO_MODE` was on and Google WIF wasn't configured. Auth is now checked first, unconditionally (commit `7512983`).
